...
<div class="modal right fade" id="editModal_{{$d->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form id="edit" action="{{ url('boards/'.$d->id) }}"  method="post" enctype="multipart/form-data" autocomplete="off">
                ...
                <img src="{{ asset('uploads/administration/board/'.$d->img) }}" alt="" srcset="" style="heigt:200px; width:200px;">
                @error('img')
                        <span class="invalid-feedback text-danger" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group">
                    ...
                    <select name="category" id="" class="form-control" value="{{ $d->category }}" required>
                        ...
                        <option value="MKB"
                            @if ($d->category=="MKB")
                                @selected(true)
                            @endif>Mwenyekiti Wa bodi
                        </option>
                        <option value="KTB"
                            @if ($d->category=="KTB")
                                @selected(true)
                            @endif>Katibu Wa Bodi
                        </option>
                        <option value="MMKB"
                            @if ($d->category=="MMKB")
                                @selected(true)
                            @endif>Makamu Mwenyekiti
                        </option>
                        <option value="MJ"
                            @if ($d->category=="MJ")
                                @selected(true)
                            @endif>Mjumbe Wa bodi
                        </option>
                    </select>
                    @error('category')
                        <span class="invalid-feedback text-danger" role="alert">
